...
| **厳密評価 AUC** | **0.6074**（全工程を fold 内に閉じ込めた時系列CV、リーク排除） |
...
当初、特徴量選択でAUC 0.72が出て喜んだが、これは**全データで特徴量重要度を計算してから交差検証する**というデータリークが生んだ幻だった。前処理・特徴量選択を含む全工程を各 fold の訓練データ内に閉じ込め、時間順を守る `TimeSeriesSplit` で測り直すと、真の実力は **AUC 0.6074** だった（ハイパーパラメータは固定のため、内側ループで探索する本来の Nested CV ではない）。
...
| 記事の章 | 場所 |
|---------|------|
| 第2〜4章（前処理・リーク排除した時系列CV・パターン比較） | `notebooks/secom_strict_analysis.ipynb`（Step 2〜5）, `src/preprocessing.py` |
