...
正直に白状すると、これまで私はデータ分析をほぼノートブックの中だけで完結させてきた。セルを上から実行して結果が出れば十分で、それを別の環境で動かすとか、コンテナにまとめて誰でも再現できるようにする、という発想そのものをあまり持っていなかった。「ノートブックで動く」こと自体がゴールだと、どこかで思っていたのだと思う。今回はじめて API 化と Docker 化に踏み込んでみて、「ノートブックで動く」と「誰の環境でも同じように動く」の間にある段差の大きさを、ようやく実感した。なぜコンテナ化や再現性がこれほど重視されるのか、その重要性を頭ではなく手で理解した、というのが今回の一番の収穫だった。この記事は、その「ノートブックの外に出る」過程の記録でもある。
...
## 2. 一番つまずいたところ：学習済みモデルの「持ち運び」

実装を始めて最初に、そして最も深くつまずいたのがここだった。先に結論を書いておくと、**ノートブックの中で定義したクラスを使って保存したモデルは、別プロセスのAPIからそのままでは読み込めない**。
...
上のCI設定で `requirements-dev.txt` を入れているのには理由がある。実は最初にCIを回したとき、ローカルでは通っていたテストが、CIだけで失敗した。原因は `httpx` というライブラリが入っていなかったことだ。FastAPI の `TestClient` は内部で `httpx` を使うが、これは**テストのときにしか要らない**依存で、APIを動かすだけのユーザーには不要だ。自分のローカルにはたまたま入っていたので気づけず、まっさらなCI環境で初めて露見した。
