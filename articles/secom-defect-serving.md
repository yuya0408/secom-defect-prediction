...
`pytest` や `httpx` を本番イメージに含める必要はないし、APIだけ使いたい人にテストツールまで入れさせるのも無駄だ。この分離は、Dockerイメージを軽く保つことにもつながる。「ローカルでは動くがCIでは動かない」は実装でよく出会う壁で、依存を役割で分けておくと、こうした取りこぼしに気づきやすくなる。
